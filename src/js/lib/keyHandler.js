class KeyHandler {
  ...
  handleKeydown = keyboardEvent => {
    var reservedKeyCodeMethod = this.keyCodeFunctionDictionary[keyboardEvent.keyCode];
  ...
  _onUpKey = () => {
    var previousCommand = this.commandLine.commandHistory.tryRotateToPreviousCommand();
    if(previousCommand) {
  ...
  _onDownKey = () => {
    var nextCommand = this.commandLine.commandHistory.tryRotateToNextCommand();
    if(nextCommand) {
      this.commandLine.clearCurrentLine();
      setTimeout(() => {
  ...
  _onTab = (keyboardEvent) => {
    ...
    if(splitUserInput[0] === 'ls' || splitUserInput[0] === 'cd') {
      let availableDirectories = this.commandLine.directoryNavigator.availableDirectoriesForInput(splitUserInput[1]);
